...
public class UserManagementController {
    ...
    @PostMapping
    public ApiResponse<Void> create(@Valid @RequestBody CreateRequest r,Authentication auth){requireRole(r.role());LocalDateTime now=LocalDateTime.now();try{jdbc.update("INSERT INTO sys_user(username,password_hash,display_name,role_code,status,failed_login_attempts,locked_at,must_change_password,version,created_at,updated_at) VALUES(?,?,?,?, 'ENABLED',0,NULL,1,0,?,?)",r.username().trim(),encoder.encode(r.password()),r.displayName().trim(),r.role(),now,now);}catch(DataIntegrityViolationException e){throw conflict("用户名已存在，请更换用户名");}audit(r.username(),r.displayName(),"CREATE","创建账号，角色："+r.role(),auth);return ApiResponse.success(null);}

    @PutMapping("/{id}")
    public ApiResponse<Void> update(@PathVariable long id,@Valid @RequestBody UpdateRequest r,Authentication auth){requireRole(r.role());int changed;try{changed=jdbc.update("UPDATE sys_user SET display_name=?,role_code=?,version=version+1,updated_at=? WHERE id=? AND version=?",r.displayName().trim(),r.role(),LocalDateTime.now(),id,r.version());}catch(DataIntegrityViolationException e){throw conflict("账号资料保存失败");}requireChanged(changed);Map<String,Object>u=user(id);audit(String.valueOf(u.get("username")),r.displayName(),"UPDATE","编辑账号，角色："+r.role(),auth);return ApiResponse.success(null);}

    @PatchMapping("/{id}/enabled")
    public ApiResponse<Void> enabled(@PathVariable long id,@Valid @RequestBody EnabledRequest r,Authentication auth){Map<String,Object>u=user(id);if(auth.getName().equals(u.get("username"))&&!r.enabled())throw conflict("不能停用当前正在登录的账号");requireChanged(jdbc.update("UPDATE sys_user SET status=?,version=version+1,updated_at=? WHERE id=? AND version=?",r.enabled()?"ENABLED":"DISABLED",LocalDateTime.now(),id,r.version()));audit(String.valueOf(u.get("username")),String.valueOf(u.get("displayName")),r.enabled()?"ENABLE":"DISABLE",r.enabled()?"恢复账号":"停用账号",auth);return ApiResponse.success(null);}

    @PatchMapping("/{id}/password")
    public ApiResponse<Void> resetPassword(@PathVariable long id,@Valid @RequestBody PasswordRequest r,Authentication auth){Map<String,Object>u=user(id);requireChanged(jdbc.update("UPDATE sys_user SET password_hash=?,must_change_password=1,failed_login_attempts=0,locked_at=NULL,version=version+1,updated_at=? WHERE id=? AND version=?",encoder.encode(r.password()),LocalDateTime.now(),id,r.version()));audit(String.valueOf(u.get("username")),String.valueOf(u.get("displayName")),"RESET_PASSWORD","管理员重置密码，下次登录必须修改",auth);return ApiResponse.success(null);}

    @PatchMapping("/{id}/unlock")
    public ApiResponse<Void> unlock(@PathVariable long id,@Valid @RequestBody VersionRequest r,Authentication auth){Map<String,Object>u=user(id);requireChanged(jdbc.update("UPDATE sys_user SET failed_login_attempts=0,locked_at=NULL,version=version+1,updated_at=? WHERE id=? AND version=?",LocalDateTime.now(),id,r.version()));audit(String.valueOf(u.get("username")),String.valueOf(u.get("displayName")),"UNLOCK","管理员解锁账号",auth);return ApiResponse.success(null);}

    private Map<String,Object>user(long id){var rows=jdbc.query("SELECT username,display_name FROM sys_user WHERE id=?",(rs,n)->map("username",rs.getString(1),"displayName",rs.getString(2)),id);if(rows.isEmpty())throw new ResponseStatusException(HttpStatus.NOT_FOUND,"账号不存在");return rows.get(0);}
    ...
    private static void requireRole(String role){if(!ROLES.contains(role))throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"角色无效");}
    private static void requireChanged(int n){if(n!=1)throw conflict("账号已被其他人修改，请刷新后重试");}
    ...
    public record CreateRequest(@NotBlank @Size(min=3,max=64) @Pattern(regexp="^[A-Za-z0-9_.-]+$",message="用户名只能包含字母、数字、点、横线和下划线")String username,@NotBlank @Size(max=100)String displayName,@NotBlank String role,@NotBlank @Pattern(regexp="^(?=.*[A-Za-z])(?=.*\\d).{8,64}$",message="密码至少8位，并同时包含字母和数字")String password){}
    ...
    public record PasswordRequest(@NotBlank @Pattern(regexp="^(?=.*[A-Za-z])(?=.*\\d).{8,64}$",message="密码至少8位，并同时包含字母和数字")String password,@NotNull Integer version){}
