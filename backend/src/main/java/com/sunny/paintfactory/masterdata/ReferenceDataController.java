...
public class ReferenceDataController {
    ...
    public ApiResponse<Void> createSupplier(@Valid @RequestBody SupplierRequest r,Authentication auth){
        ...
        catch(DataIntegrityViolationException e){throw conflict("供应商编号已存在，请更换编号");} audit("suppliers",idByCode("supplier","supplier_code",r.code()),r.code(),r.name(),"CREATE","新增供应商",auth);return ApiResponse.success(null);
    }

    @PutMapping("/suppliers/{id}")
    public ApiResponse<Void> updateSupplier(@PathVariable long id,@Valid @RequestBody SupplierRequest r,Authentication auth){
        int changed;try{changed=jdbc.update("UPDATE supplier SET supplier_code=?,short_name=?,phone=?,mobile=?,fax=?,address=?,remark=?,version=version+1 WHERE id=? AND version=?",r.code().trim(),r.name().trim(),blank(r.phone()),blank(r.mobile()),blank(r.fax()),blank(r.address()),blank(r.remark()),id,r.version());}catch(DataIntegrityViolationException e){throw conflict("供应商编号已存在，请更换编号");} requireChanged(changed);audit("suppliers",id,r.code(),r.name(),"UPDATE","编辑供应商",auth);return ApiResponse.success(null);
    }

    @PostMapping("/vehicles")
    public ApiResponse<Void> createVehicle(@Valid @RequestBody VehicleRequest r,Authentication auth){try{jdbc.update("INSERT INTO vehicle(vehicle_code,plate_no,vehicle_type,enabled,remark) VALUES(?,?,?,1,?)",r.code().trim(),blank(r.plateNo()),blank(r.vehicleType()),blank(r.remark()));}catch(DataIntegrityViolationException e){throw conflict("车辆编号或车牌号已存在，请检查后重试");}audit("vehicles",idByCode("vehicle","vehicle_code",r.code()),r.code(),r.plateNo(),"CREATE","新增车辆",auth);return ApiResponse.success(null);}

    @PutMapping("/vehicles/{id}")
    public ApiResponse<Void> updateVehicle(@PathVariable long id,@Valid @RequestBody VehicleRequest r,Authentication auth){int changed;try{changed=jdbc.update("UPDATE vehicle SET vehicle_code=?,plate_no=?,vehicle_type=?,remark=? WHERE id=?",r.code().trim(),blank(r.plateNo()),blank(r.vehicleType()),blank(r.remark()),id);}catch(DataIntegrityViolationException e){throw conflict("车辆编号或车牌号已存在，请检查后重试");}requireChanged(changed);audit("vehicles",id,r.code(),r.plateNo(),"UPDATE","编辑车辆",auth);return ApiResponse.success(null);}

    @PostMapping("/employees")
    public ApiResponse<Void> createEmployee(@Valid @RequestBody EmployeeRequest r,Authentication auth){long uid=userId(auth);LocalDateTime now=LocalDateTime.now();try{jdbc.update("INSERT INTO employee(employee_code,employee_name,gender,employee_type_id,department_id,position_name,education,id_card,address,is_salesperson,hometown,postal_code,hire_date,phone,enabled,remark,version,created_by,created_at,updated_by,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,1,?,0,?,?,?,?)",r.code().trim(),r.name().trim(),blank(r.gender()),r.employeeTypeId(),r.departmentId(),blank(r.positionName()),blank(r.education()),blank(r.idCard()),blank(r.address()),r.salesperson(),blank(r.hometown()),blank(r.postalCode()),r.hireDate(),blank(r.phone()),blank(r.remark()),uid,now,uid,now);}catch(DataIntegrityViolationException e){throw conflict("员工编号已存在或员工类别、部门无效");}audit("employees",idByCode("employee","employee_code",r.code()),r.code(),r.name(),"CREATE","新增员工",auth);return ApiResponse.success(null);}

    @PutMapping("/employees/{id}")
    public ApiResponse<Void> updateEmployee(@PathVariable long id,@Valid @RequestBody EmployeeRequest r,Authentication auth){String idSql=r.idCard()==null||r.idCard().isBlank()?"id_card":"?";List<Object>a=new ArrayList<>();Collections.addAll(a,r.code().trim(),r.name().trim(),blank(r.gender()),r.employeeTypeId(),r.departmentId(),blank(r.positionName()),blank(r.education()));if(!idSql.equals("id_card"))a.add(r.idCard().trim());Collections.addAll(a,blank(r.address()),r.salesperson(),blank(r.hometown()),blank(r.postalCode()),r.hireDate(),blank(r.phone()),blank(r.remark()),userId(auth),LocalDateTime.now(),id,r.version());int changed;try{changed=jdbc.update("UPDATE employee SET employee_code=?,employee_name=?,gender=?,employee_type_id=?,department_id=?,position_name=?,education=?,id_card="+idSql+",address=?,is_salesperson=?,hometown=?,postal_code=?,hire_date=?,phone=?,remark=?,version=version+1,updated_by=?,updated_at=? WHERE id=? AND version=?",a.toArray());}catch(DataIntegrityViolationException e){throw conflict("员工编号已存在或员工类别、部门无效");}requireChanged(changed);audit("employees",id,r.code(),r.name(),"UPDATE","编辑员工",auth);return ApiResponse.success(null);}

    @PostMapping("/{kind:employee-types|departments|routes}")
    public ApiResponse<Void> createSimple(@PathVariable String kind,@Valid @RequestBody SimpleRequest r,Authentication auth){String table=simpleTable(kind),code=simpleCode(kind),name=simpleName(kind);try{jdbc.update("INSERT INTO "+table+"("+code+","+name+",enabled,remark) VALUES(?,?,1,?)",r.code().trim(),r.name().trim(),blank(r.remark()));}catch(DataIntegrityViolationException e){throw conflict("编号已存在，请更换编号");}audit(kind,idByCode(table,code,r.code()),r.code(),r.name(),"CREATE","新增资料",auth);return ApiResponse.success(null);}

    @PutMapping("/{kind:employee-types|departments|routes}/{id}")
    public ApiResponse<Void> updateSimple(@PathVariable String kind,@PathVariable long id,@Valid @RequestBody SimpleRequest r,Authentication auth){String table=simpleTable(kind),code=simpleCode(kind),name=simpleName(kind);int changed;try{changed=jdbc.update("UPDATE "+table+" SET "+code+"=?,"+name+"=?,remark=? WHERE id=?",r.code().trim(),r.name().trim(),blank(r.remark()),id);}catch(DataIntegrityViolationException e){throw conflict("编号已存在，请更换编号");}requireChanged(changed);audit(kind,id,r.code(),r.name(),"UPDATE","编辑资料",auth);return ApiResponse.success(null);}
    ...
    public ApiResponse<Void> delete(@PathVariable String kind,@PathVariable long id,Authentication auth){
        ...
        if(references>0)throw conflict("该资料已被业务单据使用，不能删除；可以改为停用");
        ...
        Map<String,Object>label=entityLabel(kind,id);requireChanged(jdbc.update("DELETE FROM "+table+" WHERE id=?",id));audit(kind,id,String.valueOf(label.get("code")),String.valueOf(label.get("name")),"DELETE","永久删除资料",auth);return ApiResponse.success(null);
    }
    ...
    public record SupplierRequest(@NotBlank @Size(max=64) String code,@NotBlank @Size(max=200) String name,@Size(max=128) @Pattern(regexp="^[0-9+()\\-\\s]*$",message="电话格式不正确") String phone,@Size(max=64) @Pattern(regexp="^[0-9+()\\-\\s]*$",message="手机格式不正确") String mobile,@Size(max=64) String fax,@Size(max=500) String address,@Size(max=1000) String remark,@NotNull Integer version){}
    ...
    public record EmployeeRequest(@NotBlank @Size(max=64) String code,@NotBlank @Size(max=100) String name,@Size(max=20) String gender,Long employeeTypeId,Long departmentId,@Size(max=100) String positionName,@Size(max=100) String education,@Size(max=64) String idCard,@Size(max=500) String address,boolean salesperson,@Size(max=100) String hometown,@Size(max=32) String postalCode,java.time.LocalDate hireDate,@Size(max=64) @Pattern(regexp="^[0-9+()\\-\\s]*$",message="电话格式不正确") String phone,@Size(max=500) String remark,@NotNull Integer version){}
