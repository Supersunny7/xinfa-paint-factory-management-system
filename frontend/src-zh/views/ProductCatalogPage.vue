...
const currentTitle=computed(()=>selectedChild.value?`${selectedParent.value.name} · ${selectedChild.value.name}`:selectedParent.value?`${selectedParent.value.name} · 全部货品`:'全部货品')
...
<div class="catalog-layout"><el-card class="brand-panel"><template #header><strong>第一步：选择厂家</strong></template><div class="brand-list"><button v-for="item in categories" :key="item.id" class="brand-card" :class="{active:selectedParent?.id===item.id}" @click="chooseParent(item)"><span class="brand-name">{{item.name}}</span><span class="brand-code">编号 {{item.code}}</span><el-badge :value="item.productCount" :hidden="!item.productCount"/></button></div></el-card>
<div class="content-panel"><el-card class="sub-panel"><template #header><div><strong>第二步：选择 {{selectedParent?.name||''}} 的货物小类</strong><span class="hint">选“全部”可查看该厂家的所有货品</span></div></template><div class="sub-list"><button class="sub-card" :class="{active:!selectedChild}" @click="chooseChild(null)"><b>全部</b><small>{{selectedParent?.productCount||0}} 种货品</small></button><button v-for="item in selectedParent?.children||[]" :key="item.id" class="sub-card" :class="{active:selectedChild?.id===item.id}" @click="chooseChild(item)"><b>{{item.name}}</b><span>编号 {{item.code}}</span><small>{{item.productCount}} 种货品</small></button></div></el-card>
